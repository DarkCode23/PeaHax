$(document).ready(function() {
	if ($('.template__charts').length) {
		...
		// Mouse speed chart
		if ($('.template__sparkline-ms').length) {
		...
		if ($('.template__sparkline-bar').length) {
		...
		if ($('.template__sparkline-composite').length) {
				...
				case 'morris-line':
					Morris.Line({
						...
						data: [
							{ y: 'Today', a: 1, b: 0 },
							{ y: 'Yesterday', a: 2, b: 2 },
							{ y: '2 days ago', a: 3, b: 0 },
							{ y: '3 days ago', a: 7, b: 3 },
						],
						xkey: 'y',
						ykeys: ['a', 'b'],
						labels: ['BO2 Mod Pack', 'Ghosts Mod Pack'],
						lineColors: ['#ed4949', '#3DD95E'],
						pointSize: 0,
						pointStrokeColors: ['#ed4949', '#1e59d9'],
						lineWidth: 6,
						parseTime: false,
						resize: true
					});
